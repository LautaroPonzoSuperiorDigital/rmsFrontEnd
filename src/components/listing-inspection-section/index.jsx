...
import { CategoryList, Header, ListingInspectionSectionContainer, NewCategory } from "./styles"
...
export function ListingInspectionSection({ section }) {
  ...
  const inspectionTitle = useMemo(() => {
    ...
    return `${editingInspection.name} ${date} /`
  }, [editingInspection])
  ...
  const onCategoryRemoved = useCallback((removedCategoryId) => {
    setCategories(oldState => oldState.filter(category => category.id !== removedCategoryId))
  }, [])
  
  const onCategoryUpdated = useCallback((updatedCategory) => {
    const _categories = [...categories]
    const categoryIndex = _categories.findIndex(category => category.id === updatedCategory.id)
  ...
  return (
    <ListingInspectionSectionContainer>
      <Header>
        <h1 role="button" onClick={modal.close}>{inspectionTitle}</h1>
        <h1>{section.name}</h1>
      </Header>
      ...
      <Input
        ...
        containerStyle={{ marginTop: '2.5rem', alignSelf: 'flex-start', width: '35%' }}
      />
